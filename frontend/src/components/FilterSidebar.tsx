...
const inputClass =
  "w-full px-2.5 py-2.5 rounded-lg border-none bg-brand-teal text-white text-[15px] box-border"
...
export default function FilterSidebar({
  ...
  function categoryButtonClass(value: string) {
    ...
    return `w-full px-4 py-2.5 rounded-lg mb-2 flex items-center justify-between text-left text-[15px] cursor-pointer ${
      active
        ? "border-2 border-white bg-brand-teal-dark font-bold"
        : "border-2 border-transparent bg-brand-teal font-normal"
    }`
  }

  return (
    <div className="fixed right-0 top-0 w-[270px] h-full bg-brand-mint text-white p-5 overflow-y-auto z-[999]">
      <button
        onClick={onClose}
        className="w-full px-5 py-2.5 rounded-lg border-none bg-brand-teal text-white cursor-pointer text-base mb-5"
      >
        Fechar
      </button>
